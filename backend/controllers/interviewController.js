...
exports.getAllInterviews = catchAsync(async (req, res, next) => {
  ...
  const interviews = await Interview.getAllInterviews(req.user.userID, filters);
  res.status(200).json({
    status: 'success',
...
exports.getInterviewsByJob = catchAsync(async (req, res, next) => {
  ...
  const interviews = await Interview.getInterviewsByJobId(jobID, req.user.userID);
  res.status(200).json({
    status: 'success',
    results: interviews.length,
    data: { interviews },
...
exports.createInterviewForJob = catchAsync(async (req, res, next) => {
  const { jobID } = req.params;
  const newInterview = await Interview.createInterview(req.user.userID, {...req.body, jobID});
  if (!newInterview){
...
exports.getInterviewById = catchAsync(async (req, res, next) => {
  const interview = await Interview.getInterviewById(req.params.id, req.user.userID);
...
exports.updateInterview = catchAsync(async (req, res, next) => {
  const existing = await Interview.getInterviewById(req.params.id, req.user.userID);
  if (!existing) return next(new AppError('Interview not found', 404));

  const updated = await Interview.updateInterview(req.params.id, req.user.userID, req.body);
  res.status(200).json({
    status: 'success',
